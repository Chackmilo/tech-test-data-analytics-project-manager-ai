...
```bash
python verify.py         # 281 checks against the artifacts and the deliverables
...
│   ├── test_full_verification.py  # End-to-end certification (281 / 281 checks)
...
├── verify.py                      # Root backwards-compatible verification entrypoint (281 checks)
...
Run the full 281-check deterministic verification audit from the repository root:
```bash
python verify.py
# Or with quiet flag:
...
#### Makefile Automation
```bash
make verify   # Run the 281-check verification harness
